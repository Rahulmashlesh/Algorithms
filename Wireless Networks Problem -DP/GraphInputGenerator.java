...
public class GraphInputGenerator {
	...
	public GraphInputGenerator(int v, int e, boolean isWeighted,
		...
		this.edges = isDirected ? Math.min(e, v * (v - 1)) : Math.min(e, v
				* (v - 1) / 2);
		fillAdj();
	}
	...
	/**
	 * Adds a edge between vertices v1 and v2 and makes entry into the adjacency
	 * matrix accordingly. For non weighted graphs, the edge cost is 1. For non
	...
	/**
	 * Prints the graph data. First line contains number of vertices and number
	 * of edges. Following lines show edges in the graph. If the graph is
